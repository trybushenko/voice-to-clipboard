...
In Settings → Languages, Add language or Edit opens the profile editor.
Use Save to activate and persist it; defaults are in Advanced → Save Advanced.
See the [Qt Settings update guide](settings-redesign.md).
...
before recording/model loading. An incompatible Save does not change saved settings.
...
1. In Settings select/add Polish, key P, model `distil-large-v3`. Expect an
   English-only explanation; Save is rejected, existing profiles unchanged.
2. Set model `small`, Save. Expect Saved and active. Quit/relaunch:
...
4. Clear the Polish override and set default `small.en`. Save Advanced must fail without
   changing saved settings. Set default `large-v3-turbo`, Save Advanced: succeeds.
