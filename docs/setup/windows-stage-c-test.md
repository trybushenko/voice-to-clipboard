...
Набір містить 60 тестів; на Windows нормально пропускається macOS-only тест.
...
не вмикає мікрофон і не завантажує модель. На Windows тест використовує
справжні Win32 EDIT-поля. Закрий тестове вікно.
Якщо FAIL — збережи точний текст; «SendInput викликано» не замінює видимої вставки.

Додаткова автоматична перевірка захисту (вікно саме змінить поле й повернеться):

```powershell
.\.venv\Scripts\python.exe scripts/check_paste.py --auto --overlay
.\.venv\Scripts\python.exe scripts/check_paste.py --auto --change-focus
```

Перший тест перевіряє точну вставку з реальним оверлеєм. Для другого
очікуємо `PASS: switching fields and returning blocked paste.`
Це успіх захисту, а не помилка вставки. Перед повторною перевіркою L після
оновлення обов'язково `--quit` старого host, потім новий `--background`.
...
## Виправлення після Windows-фідбеку 2026-09-16

Початкове приймання C виявило реальне хибне блокування: UIA надсилав focus event
від `Shell_TrayWnd`, хоча sender не мав фокусу й активне поле не змінювалося.
Guard тепер перевіряє фактичний focused element і sender.HasKeyboardFocus.
До identity додано native keyboard-focus HWND. Помилки більше не приховують
причину відмови за загальним повідомленням про недоступний host.

Tk-поля на Windows не надають надійної окремої field identity: для них
підтримується clipboard-only. Тестове вікно тепер містить стандартні Win32 EDIT,
а CI виконує реальні positive/negative GUI перевірки, включно з оверлеєм.
При повторному збої надішли **повний новий текст помилки**, включно з причиною в кінці.


Друга причина L-відмови: старий Tk-оверлей на Windows забирав фокус під час появи.
Його замінено нативним Win32 NOACTIVATE вікном. Виправлення `a886743` пройшло
[усі CI jobs, включно з реальною вставкою з оверлеєм](https://github.com/trybushenko/voice-to-clipboard/actions/runs/35154418962).
Після pull/pip install перезапусти host; повтори обидва GUI-тести з розділу 2,
потім L у Блокноті/браузері/VS Code та L зі зміною поля. Повторювати весь
протокол для перевірки саме цього дефекту не потрібно.
